...
class dbhelper {
  ...
  static Stream<QuerySnapshot<Map<String, dynamic>>> getBusListbyroute(
          String? city, String? city1, String startTime) =>
      db
          .collection(collectionSchedule)
          .where('$scheduleFieldfrom', isEqualTo: city)
  ...
  static Future<void> addRequestBus(RequestModel requestBus) {
    return db
        ...
        .doc(requestBus.reqId)
        .set(requestBus.toMap());
  }
  ...
  static Future<void> deleteRequestById(String? reqId) =>
      db.collection(collectionRequest).doc(reqId).delete();
  ...
  static Future<void> deletenotificationUserById(String? reqId) =>
      db.collection(collectionNotificationUser).doc(reqId).delete();
  ...
  static Future<DocumentSnapshot<Map<String, dynamic>>> getAdminById(
          String id) =>
      db.collection(collectionUser).doc(id).get();



  static Future<void> addUserTicket(String uid, TicketUserModel userTicket) {
    return db
        .collection(collectionUser)
        .doc(uid)
        .collection(collectionticketofUser)
        .doc(userTicket.id)
        .set(userTicket
        .toMap());
  }
  static Stream<QuerySnapshot<Map<String, dynamic>>> getAllTicketByUser(
      String uid) =>
      db
          .collection(collectionUser)
          .doc(uid)
